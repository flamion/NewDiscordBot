...
public class CustomPlayerManager extends DefaultAudioPlayerManager implements AudioPlayerManager {
    ...
    public Future<Void> loadItem(final String identifier, final CustomAudioLoadResultHandler resultHandler, int index) {
        try {
            return trackInfoExecutorService.submit(createItemLoader(identifier, resultHandler, index));
        } catch (RejectedExecutionException e) {
            return handleLoadRejected(identifier, resultHandler, e);
        }
    }

    private Future<Void> handleLoadRejected(String identifier, CustomAudioLoadResultHandler resultHandler, RejectedExecutionException e) {
    ...
    private Callable<Void> createItemLoader(final String identifier, final CustomAudioLoadResultHandler resultHandler, int index) {
        return () -> {
            boolean[] reported = new boolean[1];

            try {
                if (!checkSourcesForItem(new AudioReference(identifier, null), resultHandler, reported, index)) {
    ...
    private void dispatchItemLoadFailure(String identifier, CustomAudioLoadResultHandler resultHandler, Throwable throwable) {
    ...
    private boolean checkSourcesForItem(AudioReference reference, CustomAudioLoadResultHandler resultHandler, boolean[] reported, int index) {
        ...
        for (int redirects = 0; redirects < MAXIMUM_LOAD_REDIRECTS && currentReference.identifier != null; redirects++) {
            AudioItem item = checkSourcesForItemOnce(currentReference, resultHandler, reported, index);
            if (item == null) {
                return false;
    ...
    private AudioItem checkSourcesForItemOnce(AudioReference reference, CustomAudioLoadResultHandler resultHandler, boolean[] reported, int index) {
        for (AudioSourceManager sourceManager : sourceManagers) {
